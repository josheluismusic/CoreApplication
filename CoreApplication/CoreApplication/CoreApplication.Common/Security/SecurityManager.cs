...
using CoreApplication.Common.Security.Providers;
...
namespace CoreApplication.Common.Security
{
    public static class SecurityManager
    {
        ...
        /// <returns>Verdader si la identidad se encuentra asociada al rol</returns>
        public static bool IsUserInRole(NucleoIdentity identity, string role)
        {
            ...
            var permissions = PermissionsCache.GetOrAdd<IEnumerable<string>>(identity.Name, () =>
                {
                    return SecurityManager.AuthorizationProvider.GetPermissions(identity.Name).ToList();
                });
